...
echo "****** Inicio reporte_grupo.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio reporte_grupo.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
cmdExec="groupUpdater.ReportGroup"
cmd="$JAVA_8_HOME/bin/java -cp "$cmdClassPath" "$cmdExec

cd $dirBase
if [[ $# -lt 1 ]]; then
	echo "Parametros insuficientes"
	echo "Modo de uso: ./reporte_grupo.sh <ID Grupo Internacional>"
	exit 1
fi
