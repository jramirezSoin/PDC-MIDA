...
echo "Inicio alta_campana.sh: "`date +%Y/%m/%d_%H:%M:%S` - $* >> $lockfile
echo "****** Inicio alta_campana.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio alta_campana.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
if [[ $# -lt 10 ]]; then
	...
	echo "Modo de uso: ./alta_campana.sh <Tipo de campana> <Tipo de tasacion> <ID campana> <Nombre en la factura> <Marcacion real> <Fecha inicio> <Fecha final> <GLID> <Precio> <Tax Code>"
	echo "Donde:"
