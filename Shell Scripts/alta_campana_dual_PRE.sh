...
echo "Inicio alta_campana_dual_PRE.sh: "`date +%Y/%m/%d_%H:%M:%S` - $* >> $lockfile
echo "****** Inicio alta_campana_dual_PRE.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio alta_campana_dual_PRE.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
if [[ $# -lt 10 ]]; then
	...
	echo "Modo de uso: ./alta_campana_dual_PRE.sh <Modalidad> <ID campana> <Nombre en la factura> <Fecha inicio> <Fecha final> <GLID MO> <GLID MT> <Precio MO> <Precio MT> <Tax Code MO> <Tax Code MT>"
	echo "Donde:"
