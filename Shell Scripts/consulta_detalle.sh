...
echo "****** Inicio consulta_detalle.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio consulta_detalle.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
cmdExec="markmanager.ReportMarkDetail"
cmd="$JAVA_8_HOME/bin/java -cp "$cmdClassPath" "$cmdExec

cd $dirBase
if [[ $# -lt 1 ]]; then
	echo "Parametros insuficientes"
	echo "Modo de uso: ./consulta_detalle.sh <ID Campaña>"
	exit 1
fi
