...
if [[ $# -lt 1 ]]; then
	...
	echo "Modo de uso: ./initial_setup.sh <Elemento a extraer>"
	echo "Donde:"
	echo "	 Elemento a extraer: -items / -charges / -zonning / -usc / -all / -history / -prepaid -/hybrid"
	echo "Nota: -all incluye: -items, -charges, -usc y -zonning"
	exit 1
fi

echo "****** Inicio initial_setup.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio initial_setup.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
cmdExec="markmanager.InitialSetup"
