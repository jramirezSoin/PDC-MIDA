...
echo "****** Inicio modificar_numero.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
print "Inicio modificar_numero.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
cmdExec="markmanager.ModRealNumber"
cmd="$JAVA_8_HOME/bin/java -cp "$cmdClassPath" "$cmdExec

cd $dirBase
if [[ $# -lt 3 ]]; then
	echo "Parametros insuficientes"
	echo "Modo de uso: ./modificar_numero.sh <ID campana> <Numero real> <Fecha desde>"
