...
echo "****** Inicio modificar_precio_PRE.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio modificar_precio_PRE.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
if [[ $# -lt 7 ]]; then
	...
	echo "Modo de uso: ./modificar_precio_PRE.sh <Modalidad> <Tipo de campana> <Tipo de tasacion> <ID campana> <Precio> <Fecha desde> <Tax Code> <GLID>"
	echo "Donde:"
