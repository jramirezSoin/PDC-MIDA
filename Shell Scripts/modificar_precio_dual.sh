...
echo "****** Inicio modificar_precio_dual.sh: "`date +%Y/%m/%d_%H:%M:%S`" ******"
echo "Inicio modificar_precio_dual.sh: "`date +%Y/%m/%d_%H:%M:%S` >> bitacora.txt
...
if [[ $# -lt 8 ]]; then
	...
	echo "Modo de uso: ./modificar_precio_dual.sh <ID campana> <Precio MO> <Precio MT> <Fecha desde> <GLID MO> <GLID MT> <Tax Code MO> <Tax Code MT>"
	echo "Donde:"
